...
class DatasetCreator:
    def __init__(self, markers_dir, irrelevant_images_dir, output_dir, total_images, images_per_label=2):
        ...
        self.marker_folders = [os.path.join(markers_dir, f) for f in os.listdir(markers_dir) if os.path.isdir(os.path.join(markers_dir, f))]
        self.all_marker_images = {folder: self.list_images(folder) for folder in self.marker_folders}
    ...
    def paste_images(self, background_image, images_to_paste, labels, image_index):
        ...
        for img_path, label in zip(images_to_paste, labels):
            ...
            for _ in range(100):  # Try up to 100 times to find a non-overlapping position
                ...
                if not any(self.overlaps(new_area, pasted_area) for pasted_area in pasted_areas):
                    break
            else:
            ...
            label_position = self.calculate_label_positions(background.width, background.height, img, x, y)
            label_data.append((label, *label_position))

        background = background.convert("RGB")
        final_image_path = os.path.join(self.output_dir, f'dataset_image_{image_index}.jpg')
    ...
    def create_dataset(self):
        ...
        for image_index in tqdm(range(self.total_images), desc="Creating dataset:"):
            ...
            background_image = self.all_irrelevant_images.pop() if self.all_irrelevant_images else random.choice(self.list_images(self.irrelevant_images_dir))
            ...
            for folder in self.marker_folders[:num_folders_to_select]:
                ...
                if folder_images:  # Check if there are unused images in the folder
                    selected_from_folder = [folder_images.pop() for _ in range(min(len(folder_images), self.images_per_label))]
                else:  # If all images in the folder have been used, randomly select
                    selected_from_folder = random.sample(self.list_images(folder), self.images_per_label)
    ...
    def split_dataset(self, train_ratio=0.8):
        ...
        def move_files(files, destination):
            for f in files:
                # Move image
                shutil.move(os.path.join(self.output_dir, f), os.path.join(destination, f))
                # Move corresponding label file
                label_file = f.replace('.jpg', '.txt')
                shutil.move(os.path.join(self.output_dir, label_file), os.path.join(destination, label_file))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Create dataset and split into training and validation sets.')

    # Add arguments
    parser.add_argument('--markers', type=str, required=True, help='Path to directory containing markers.')
    parser.add_argument('--irrelevant', type=str, required=True, help='Path to directory containing irrelevant images.')
    parser.add_argument('--output', type=str, required=True, help='Path to save the output dataset.')
    parser.add_argument('--total_images', type=int, default=1000, help='The total desired number of images in the dataset.')
    parser.add_argument('--train_ratio', type=float, default=0.8, help='Ratio of train data to total data.')

    # Parse arguments from command-line
    args = parser.parse_args()

    # Use the command line arguments in the program
    creator = DatasetCreator(args.markers, args.irrelevant, args.output, args.total_images)
